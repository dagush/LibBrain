...
def saveRSN_labels(idxs, outFile):
    # header = ["RSN Label"]
    with open(outFile, 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        # writer.writerow(header)
...
def RSN_Glasser360():
    ...
    outPath = WBF.WorkBrainProducedDataFolder + '_Parcellations/Glasser360RSN.csv'
    ...
    outPath = WBF.WorkBrainProducedDataFolder + f'/_Parcellations/Glasser360RSN_{"14" if useLR else "7"}_RSN_labels.csv'  # if we do NOT use detailed regions
    saveRSN_labels(names, outPath)
    ...
    outPath = WBF.WorkBrainProducedDataFolder + f'/_Parcellations/Glasser360RSN_{"14" if useLR else "7"}_indices.csv'  # if we do NOT use detailed regions
