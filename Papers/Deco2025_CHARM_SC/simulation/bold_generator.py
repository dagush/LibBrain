...
try:
    ...
    # Fallback: minimal FC implementation using numpy directly.
    # Used when neuronumba is not installed (e.g. during testing).
    # The NeuroNumba FC observable is just corrcoef(signal, rowvar=False)
    # with the same (T, N) input convention.
    import numpy as _np
    ...
    class _FCObservable:
        ...
        def from_fmri(self, bold_signal):
            # bold_signal: (T, N) — rows = timepoints, cols = ROIs
            with _np.errstate(invalid='ignore'):   # suppress NaN warnings
                fc = _np.corrcoef(bold_signal, rowvar=False)
            return {'FC': fc}
