...
# =================================================================
def mapValues2Colors(values, cmap, **kwds):
    # Convert values to colors: normalize and map to a color palette
    vmin = values.min() if 'vmin' not in kwds else kwds['vmin']
    vmax = values.max() if 'vmax' not in kwds else kwds['vmax']
...
def plotColorView(ax, cortex, data, numRegions,
                  viewkey,  # The name of the view to display: 'Lh-lateral', 'Lh-medial', 'Rh-medial',
                            # 'Rh-lateral', 'L-superior', 'R-superior', 'L-inferior', 'R-inferior'
                            # 'L-flat', 'R-flat'
                  mode='gouraud',
                  shadowed=True,
    ...
    views = {
        ...
        'L-superior': Triangulation(-xL, yL, tri_L[::-1]),
        'R-superior': Triangulation(-xR, yR, tri_R[::-1]),
        'L-inferior': Triangulation(xL, yL, tri_L),
        'R-inferior': Triangulation(xR, yR, tri_R),
    ...
    if 'map_L' in cortex or 'map_R' in cortex:
        ...
        rm_L = cortex['map_L']
        rm_R = cortex['map_R']
        if viewkey in ['Lh-lateral', 'Lh-medial', 'L-superior', 'L-inferior']:
    ...
    if mode != 'gouraud' or 'flat' in viewkey:  # either 'L-flat' or 'R-flat'
        ...
        colors = mapValues2Colors(vvalues, cmap, **kwds)  # Vertex colors
